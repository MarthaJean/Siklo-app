...
const handleCancel = async () => {
  ...
  if (
    ...
    formData.value.image_url.startsWith(`${supabaseImageBaseUrl}announcements/`)
  ) {
    try {
...
const uploadToSupabase = async (file: File) => {
  ...
  try {
    ...
    // Get the public URL
    const publicUrl = `${supabaseImageBaseUrl}announcements/${data.path}`;
...
const removeImage = async () => {
  try {
    ...
    if (
      formData.value.image_url &&
      formData.value.image_url.startsWith(
        `${supabaseImageBaseUrl}announcements/`,
      )
    ) {
      // Extract filename from URL
      const urlParts = formData.value.image_url.split("/");
